...
export const clerkWebhooks = async (req, res) => {
  try {
    ...
    // Verify webhook only if headers exist (skip for manual testing)
    if (req.headers["svix-id"] && req.headers["svix-signature"]) {
      ...
      console.warn("Skipping Svix verification (manual/test request)");
    }
    ...
    switch (type) {
      case 'user.created': {
        const userData = {
          ...
          name: `${data.first_name || "NoFirst"} ${data.last_name || "NoLast"}`,
          imageUrl: data.image_url || "",
          enrolledCourses: []
        };
